...
// This is needed because the http module has some incompatibility
// with dnt, however we only need this single module
...
export class Server {
	 ...
	 */
	async #respond(requestEvent: Deno.RequestEvent, connInfo: ConnInfo) {
				...
				if (
					...
					try {
						await delay(acceptBackoffDelay, {
							signal: this.#acceptBackoffDelayAbortController
								.signal,
						});
					} catch (err: unknown) {
